	...
	virtual bool do_start() //connect or receive
	{
		if (!ST_THIS stopped())
		{
			if (ST_THIS reconnecting && !ST_THIS is_connected())
				...
				ST_THIS next_layer().async_handshake(boost::asio::ssl::stream_base::client, ST_THIS make_handler_error([](const boost::system::error_code& ec) {ST_THIS handshake_handler(ec);}));
			else
				ST_THIS do_recv_msg();
